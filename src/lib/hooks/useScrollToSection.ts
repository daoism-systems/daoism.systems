...
import { buildSectionTimelines, SECTION_REVEAL_COMPLETE } from '$lib/config/sectionTimeline';
...
// Land each section at its reveal-complete point — matching the scroll indicator,
// which selects a section's tick the moment that section is fully revealed.
const SECTION_NAV_TARGET_PROGRESSES = SECTION_TIMELINES.map((section, index) => {
	const reveal = SECTION_REVEAL_COMPLETE[index] ?? 0;
...
export function useScrollToSection() {
	...
	function scrollToSection(
		...
		const targetProgress = clamp01(
			SECTION_NAV_TARGET_PROGRESSES[idx] ?? targetSection.timelineStart
		);
